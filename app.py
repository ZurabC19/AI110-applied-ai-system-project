...
if "owner" not in st.session_state:
    ...
    st.session_state.owner = loaded  # None if file doesn't exist
...
tab1, tab2, tab3, tab4, tab5 = st.tabs([
    "📅 Daily Schedule", "➕ Add Task", "✅ Complete Task", "🧠 Smart Tools", "🐾 My Pets"
])
...
with tab4:
    ...
    with col_a:
        ...
        duration_req = st.number_input("Task duration needed (min)", min_value=5, max_value=120, value=30, step=5)
        if st.button("Find Slot"):
            slot = scheduler.next_available_slot(duration_req)
            if slot:
                st.success(f"Next free slot: **{slot}**")
            else:
                st.warning("No free slots found between 07:00–21:00.")

    with col_b:
        st.markdown("#### ⚖️ Weighted Priority Rankings")
    ...
    if results:
        for pet_name, task in results:
            ...
            st.markdown(f"{icon} **[{pet_name}]** {task.time} — {task.description} ({task.frequency})")
    else:
        ...
        with st.expander(f"{pet.name} — {pet.species}, age {pet.age}"):
            if not pet.tasks:
                ...
                for task in pet.tasks:
                    icon = "✅" if task.completed else "⏳"
                    st.markdown(
                        f"{icon} **{task.time}** — {task.description} "
